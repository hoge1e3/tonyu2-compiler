...
export class ParserContext {
	constructor(public space: SpaceSpec){}
...
export class Parser {// class Parser
	...
	andNoUnify(next:Parser) {// Parser.and:: (Function|Parser)  -> Parser
		...
		return res.setName("("+nelems.map((p:Parser)=>p.name).join(" ")+")",{type:"and", elems:nelems});
	}
...
//        		this._first={space: space, chars:ct};
			...
			return this.context.fromFirst({[ALL]:this}).setAlias(this);
	...
	firstTokens (tokens:string|string[]/*|symbol*/) {
		...
		if (typeof tokens=="symbol") {
			if (tokens!==ALL) throw new Error("except ALL not allowed ");
			tbl[ALL]=this;
		} else {
			const tka:string[]=(typeof tokens=="string" ? [tokens] : tokens);
	...
	unifyFirst (other:Parser) {
		...
		var res=this.context.fromFirst(tbl).setName(`(${nelems.map((p:Parser)=>p.name).join("|")})`,{type:"or",elems:nelems});
	...
	orNoUnify (other:Parser) {
		...
		}).setName(`(${nelems.map((p:Parser)=>p.name).join("|")})`,{type:"or", elems:nelems} );
		return res;
	}
	setAlias(p:Parser){
	...
	obj(...names:(string|null)[]):Parser {
		...
		return this.ret((...args:any[])=>{
			...
			for (let name in fields) {
				...
				if (idx<0 || idx>=args.length) throw new Error("Index out");
...
export const tokensParserContext=new ParserContext("TOKEN");
export const TokensParser={
	context: tokensParserContext,
	create(pf:ParseFunc) {
		return tokensParserContext.create(pf);
	},
	token(type:string) {
	...
	parse:function (parser:Parser, tokens:any[], global:any={}) {
...
export function lazy(context: ParserContext, pf:()=>Parser):Parser {
	...
	function resolve() {
		if (!lz.resolved) {
			lz.resolved=pf();
			if (!lz.resolved) throw new Error(pf+" returned null!");
